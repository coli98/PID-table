...
name_file= 'Obie osie_' + t +'.csv'

with open(name_file, 'w', encoding='utf-8') as csvfile:
    csvwriter = csv.writer(csvfile)
    csvwriter.writerow(['czas[s]', 'pozycja_x[pixel]', 'wyjscie regulatora_x', 'bład errox_x', 'pozycja_y[pixel]','wyjscie regulatora y','bład error_y'])
    ...
    while True:
        ...
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow([round(elapsed_time,2), x_medium, round(output), error_x, y_medium, round(output_y), error_y]) #, output])
